(function($) {
    ...
    $(document).ready(function() {
        $(document).on('keyup change', '#search_form_input', function(event) { let input_values = $(this).val(); if (input_values.length > 0) { $('#search_suggestions_wrap, .search-suggestion-overlay').addClass("show"); } else { $('#search_suggestions_wrap, .search-suggestion-overlay').removeClass("show"); } });
        $(document).on('click', '.search-suggestion-overlay, .search-click-icon', function() { $('#search_suggestions_wrap, .search-suggestion-overlay').removeClass('show') })
        ...
        $(document).on('click', '.navbar-toggler', function() { $(".navbar-toggler").toggleClass("active"); });
        $(document).on('click', '.click-nav-right-icon', function() { $(".show-nav-content").toggleClass("show"); });
        $(document).on('click', '.click-content-show', function() { $(".right-contents-show").toggleClass("show"); });
        $(document).on('click', '.shop-close-content-icon, .responsive-overlay', function() { $('.shop-close-content, .responsive-overlay').removeClass('active'); });
        $(document).on('click', '.shop-icon-sidebar', function() { $('.shop-close-content, .responsive-overlay').addClass('active'); });
        $('.gallery-popup, .gallery-popup-two').magnificPopup({ type: 'image', gallery: { enabled: true, } });
        $('.imgLoaded').imagesLoaded(function() { $('.masonry-grid').isotope({ itemSelector: '.grid-item', percentPosition: true, masonry: { columnWidth: '.grid-item' } }) });
        ...
        $(function() {
            ...
            $(document).on('click', '.minus', function() { var selectedInput = $(this).next('.quantity-input'); if (selectedInput.val() > 1) { selectedInput[0].stepDown(1); } });
        });
        ...
        $(document).on('click', '.popup-overlay, .popup-close', function() { $('.popup-fixed, .popup-overlay').removeClass('popup-active'); });
        $(document).on('click', '.popup-click', function() { $('.popup-fixed, .popup-overlay').toggleClass('popup-active'); });
        ...
        $(document).on('click', '.close-bars, .body-overlay', function() { $('.dashboard-close, .dashboard-left-content, .body-overlay').removeClass('active'); });
        $(document).on('click', '.sidebar-icon', function() { $('.dashboard-close, .dashboard-left-content, .body-overlay').toggleClass('active'); });
        ...
        if (void 0 !== i && null !== i) {
            ...
            noUiSlider.create(i, { start: [j, k], connect: !0, step: n, range: { min: l, max: m } }), i.noUiSlider.on("update", function(a, b) {
                var c = a[b];
        ...
        $(document).on("click", ".toggle-password", function(e) {
            ...
            if (inputPass.attr("type") == "password") { inputPass.attr("type", "text"); } else { inputPass.attr("type", "password"); }
        });
        /* if (document.querySelector('#phone') != null) {
            let TellInput = document.querySelector("#phone");
            window.intlTelInput(TellInput, { allowDropdown: true, excludeCountries: ["in"], });
        } */
        $(document).on('click', '.custom-radio-single', function(e) {
            $(this).siblings().removeClass('active');
            $(this).addClass('active');
        });
        $(".date-picker").flatpickr({ mode: "single", enableTime: true, dateFormat: "d-m-Y H:i", altInput: true, altFormat: "F j, Y", time_12hr: true, });
        ...
        if (globalSlickInit.length > 0) {
            $.each(globalSlickInit, function(index, value) {
                if ($(this).children('div').length > 1) {
                    ...
                    if (centerMode != false) { sliderSettings.centerMode = centerMode; }
        ...
        $(document).on('click', '.back-to-top', function() { $("html,body").animate({ scrollTop: 0 }, 700); });
    });
    $(window).on('scroll', function() { var ScrollTop = $('.back-to-top'); if ($(window).scrollTop() > 200) { ScrollTop.fadeIn(10); } else { ScrollTop.fadeOut(10); } });
})(jQuery);