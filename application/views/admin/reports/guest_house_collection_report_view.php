    ...
    <div class="app-content pt-3 p-md-3 p-lg-3">
        <div class="container-xl">
            <div class="row g-3 mb-2 align-items-center justify-content-between">
                <div class="col-auto">
                    <h1 class="app-page-title mb-0">Collection Report</h1>
                </div>
                <div class="col-auto">
            ...
            <div class="app-card app-card-settings shadow-sm p-3">
                <div class="app-card-body">
					...
					<input type="hidden" name="<?= $this->csrf['name']; ?>" value="<?= $this->csrf['hash']; ?>">
                        <div class="row mb-3">
							<div class="col-lg-12 col-sm-12 col-md-12 form-label">
								Selection of a Property is always compulsory.<br />
								Only selection of a Property and not a POS will generate Collection Report of Guest House Booking.<br />
								If you want to get Collection Report of a POS then only select the same.
							</div>
						</div>
						<div class="row g-3">
                        ...
                        <table class="table align-middle app-table-hover mb-0 pt-1 small w-100" id="collection_list">
							...
							?>
								<tr>
									...
									<td><?= date('H:i:s', strtotime($row['payment_date']));?></td>
									<td><?= $row['property_name'];?></td>
									<td><?= $row['cost_center_name'];?></td>
									<td><?= ($row['sale_order_id'] == '') ? $row['booking_no'] : '';?></td>
									<td><?= ($row['booking_source'] == "B") ? (($row['check_in_id'] != '') ? $row['check_in_id'] : 'Not checked in') : '';?></td>
									<td><?= ($row['sale_order_id'] != '') ? $row['invoice_no'] : '';?></td>
									<td><?= $row['order_id'];?></td>
    ...
    $('#collection_list').DataTable( {
       ...
       "bPaginate": false,
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            $( api.column( 11 ).footer() ).html(amountTotal.toFixed(2));
        },
        buttons: [{
            "extend": 'excel',
            "footer": true,
            ...
            'filename': 'Guest_House_Collection_Report'+ today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate()+'_'+today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds(),
            },
            {
            "extend": 'csv',
            "footer": true,
