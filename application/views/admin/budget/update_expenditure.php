...
<div class="app-content pt-3 p-md-3 p-lg-4">
		<div class="container">
			<div class="row g-3 mb-3 align-items-center justify-content-between">
				<div class="col-auto">
					<h1 class="app-page-title mb-0">Add / Update Expenditure</h1>
				</div>
				<div class="col-auto">
					<div class="page-utilities">
						<div class="row g-2 justify-content-start justify-content-md-end align-items-center">
							<!--//col-->
							<div class="col-auto">
								<a class="btn app-btn-primary" href="<?= base_url(); ?>admin/budget/actual_expenditure">
									Back
								</a>
							</div>
						</div>
			...
			<form id="expenditureForm" action="<?= base_url(); ?>admin/budget/expenditure_submit" method="post" enctype="multipart/form-data">
				...
				<div class="app-card app-card-settings shadow-sm mb-3 pt-2">
					<div class="app-card-body pb-3">
						<h6 class="p-3"><span class="mainDate">Actual Expenditure for the Month of: <?= $fYear; ?></span></h6>
						<div class="table-responsive">

							<button style="background: #00bdd6; padding: 6px 10px; color: #FFF; border-radius: 6px;" id="printAcc">Print</button>

							<table id="estiDetailtab" class="table table-bordered align-middle table-hover" style="display:none;">
								<thead class="small" style="background-color: #608d5f;">
									<tr>
										...
										<th width="" class="cell text-white text-end"><span style="display: block; font-size: 11px;">Actual Expenditure</span> Amount (In Rs.)</th>										
									</tr>
								</thead>
								<tbody>
									...
									<?php foreach($estimation_details['budget_details'] as $estimation){ ?>
										...
										<tr>
											...
											<td width="" class="cell"><?php if(!empty($estimation['actual_expenditure_amount'])){ echo $estimation['actual_expenditure_amount']; } else { echo '0.00'; } ?></td>
										</tr>
									...
									<tr style="background-color: #1a4919; font-size: 1.1rem;">
										...
										<td width="" class="cell text-white text-end total_estimated_amount_text"><?php if(!empty($estimation_details['estimated_expence_total'])){ echo $estimation_details['estimated_expence_total']; } else { echo '0.00'; } ?></td>
										<td width="" class="cell text-white text-end total_approved_amount_text"><?php if(!empty($estimation_details['approved_expence_total'])){ echo $estimation_details['approved_expence_total']; } else { echo '0.00'; } ?></td>
										<td width="" class="cell text-white text-end total_actual_expenditure_amount_text"><?php if(!empty($estimation_details['actual_expenditure_total'])){ echo $estimation_details['actual_expenditure_total']; } else { echo '0.00'; } ?></td>                                       
									</tr>
								</tbody>
							</table>

							<table id="estiDetailtab1" class="table table-bordered align-middle table-hover" style="border:1px solid #e7e9ed;">
								<thead class="small" style="background-color: #608d5f;">
									<tr>
										<th width="" class="cell text-white">Particulars</th>
										<th width="" class="cell text-white text-end">Amount (Rs.)</th>
								...
								<tbody>
									...
									<?php foreach($estimation_details['budget_details'] as $estimation){ ?>
										...
										<tr>
											...
											<td width="" class="cell">
													<input type="text" class="form-control text-end fw-bold actual_expenditure_amount" name="actual_expenditure_amount[]" value="<?= $estimation['actual_expenditure_amount']; ?>" placeholder="0.00">
												
											</td>

                                            <td width="" class="cell">

                                                    <input type="file" multiple class="form-control expenditure_files" name="expenditure_files[<?= $i; ?>][]">
									...
									<tr style="background-color: #1a4919; font-size: 1.1rem;">
                                        ...
                                        <input type="hidden" name="total_actual_expenditure_amount" class="total_actual_expenditure_amount" value="<?php if(!empty($estimation_details['actual_expenditure_total'])){ echo $estimation_details['actual_expenditure_total']; } else { echo '0.00'; } ?>">
									</tr>
								</tbody>
							</table>
						</div>
						<div class="row">
							...
							<div class="col-md-12 px-4 text-end">
								<input type="submit" class="btn app-btn-primary submitExpenditure" name="" value="Submit">
							</div>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	$( document ).ready(function() {
        ...
        $(document).on('input', '.actual_expenditure_amount', function() {
			// Initialize the total sum to zero
			var total = 0;

			// Loop through each input field
			$('.actual_expenditure_amount').each(function() {
				// Parse the value as a float and add it to the total
				total += parseFloat($(this).val()) || 0;
			});

			// Update the result element with the calculated total
			$('.total_actual_expenditure_amount_text').text(total.toFixed(2));
			$('.total_actual_expenditure_amount').val(total.toFixed(2));
        ...
        $(document).on('click', '.submitExpenditure', function(e) {
            ...
            $.confirm({
                ...
                title: "Alert!!",
                content: "Are you sure to submit details? Please check details before submit.",
                buttons: {
                    Ok: {
                        text: 'Yes',
                        btnClass: 'btn-green',
