<div class="app-content pt-3 p-md-3 p-lg-3">
            ...
            <?php endif ?>
                <div class="row g-3 mb-4 align-items-center justify-content-between">
                    <div class="col-auto">
                        <h1 class="app-page-title mb-0">Registration Fee / Monthly Subscription</h1>
                    </div>
                    <div class="col-auto">
                ...
                <div class="app-card app-card-orders-table shadow-sm mb-5 p-4">
                    <div class="app-card-body">
                        <form class="settings-form" method="post" action="<?= base_url('admin/gymnasiumrate/submitgymnasiumrate'); ?>" enctype="multipart/form-data" autocomplete="off">
                            <div class="row g-3">
                                ...
                                <div class="col-sm-12 col-md-4 mb-3">
                                    <label for="" class="form-label">Gymnasium<span class="asterisk"> *</span></label>
                                    <select name="sports_facilities_id" class="form-select" id="sports_facilities_id" required>
                                        <option value="" selected disabled>Select Gymnasium</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row g-3">
    ...
    <script>
        $(document).ready(function() {
            ...
            var fieldunit_id = $(this).val();
                ...
                $.ajax({
                    url:'<?= base_url("admin/Gymnasiumrate/getlocation"); ?>',
                    method: 'post',
            ...
            var slug = 'Gymnasium facilities';
                ...
                $.ajax({
                    url:'<?= base_url("admin/Gymnasiumrate/getGymnasiums"); ?>',
                    method: 'post',
                    data: {location_id: location_id, slug: slug},
                    dataType: 'json',
                    success: function(response){
                    var resultHTML = '<option value="" selected disabled>Select Gymnasium</option>';
                    $.each(response,function(index,data){
        ...
        $('#sports_facilities_id').change(function(){
            ...
            $.ajax({
                url:'<?= base_url("admin/Gymnasiumrate/getpreviousrate"); ?>',
                method: 'post',
                ...
                var i =0;
                if(response.status){
                    $.each(response.gymnasium_rates,function(key,value){i++;
                    ...
                    resultHTML +=`<tr>
                                ...
                                <td class="cell">`;
                                    if(i == 1){
                                        resultHTML +=`<a class="btn-sm app-btn-secondary" href="<?=base_url('admin/Gymnasiumrate/edit_rate/')?>`+value.gymnasium_rate_id+`">Edit</a>`;
                                        } 
                                        resultHTML +=`</td>
                            </tr>`;
                ...
                $('#gymnasium_list').html(resultHTML);
                }
            });
        });
    </script>