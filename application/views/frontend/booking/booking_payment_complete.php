							...
							<div class="col-lg-12 col-md-12 col-sm-12 text-center">
								<div style="width: 80%; margin:0 auto; padding-top: 30px;" class="text-center">
									<?php if ($status == 'SUCCESS') { ?>
										<h4 class="text-success">Congratulations! Your Payment is Successful and Booking is confirmed.<br><br><?= isset($booking_det->booking_no) && $booking_det->booking_no != '' ? 'Your Booking No. ' . $booking_det->booking_no : ''; ?></h4>
									<?php } elseif ($status == 'FAILURE') { ?>
										<h4 class="text-danger">Payment Failed. Please try again.</h4>
									<?php } elseif ($status == 'CANCELED') { ?>
										<h4 class="text-danger">Payment Canceled. Please try again.</h4>
									<?php } ?>
								</div>
							</div>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-body">
				<?php if ($status == 'SUCCESS') { ?>
					<h6 class="text-success">Congratulations! Your Payment is Successful and Booking is Confirmed.<br><br><?= isset($booking_det->booking_no) && $booking_det->booking_no != '' ? 'Booking No. ' . $booking_det->booking_no : ''; ?><?= isset($booking_det->transaction_ref_id) && $booking_det->transaction_ref_id != '' ? '<br>Transaction Ref. No. ' . $booking_det->transaction_ref_id : ''; ?><?= isset($booking_det->order_id) && $booking_det->order_id != '' ? '<br>Order Number ' . $booking_det->order_id : ''; ?><?= isset($booking_det->amount) && $booking_det->amount != '' ? '<br>Amount ₹' . $booking_det->amount : ''; ?></h6>
				<?php } elseif ($status == 'FAILURE') { ?>
