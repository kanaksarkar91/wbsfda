<section class="section">
    <div class="container text-center">
        <h4>Welcome <?= $this->session->userdata('contact_person')?> on behalf of</h4>
        <h3 class="title" style="color:#800000;font-weight:bold;"><?= $this->session->userdata('organization_name')?></h3>
                                        ...
                                        <tr>
                                            <td>
                                                <label class="check_cont">
                                                    <input type="checkbox" class="checkboxClass" name="check" value="<?= $date?>">
                                                    <span class="checkmark"></span>
                                                </label>
                                            </td>
                                            <td class="text-center"><?= date('d-m-Y D',strtotime($date))?></td>
                ...
                <div class="modal-body" style="padding: 10px;">
                    <table class="table table-bordered">
                        <tr>
                            ...
                            <td><?= $facilities['sports_facilities_name']?></td>
                            <input type="hidden" name="sports_facilities_id" id="sports_facilities_id" value="<?= $facilities['sports_facilities_id']?>">
                        </tr>
                        <tr>
                            <th>Location :</th>
                            <td><?= $facilities['location_name']?></td>
                            <input type="hidden" name="location_name" id="location_name" value="<?= $facilities['location_name']?>">
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td><?= $this->session->userdata('organization_name')?></td>
                            <input type="hidden" name="organization_name" id="organization_name" value="<?= $this->session->userdata('organization_name')?>" >
                        </tr>
                        <tr>
                            <th>Mailing Address :</th>
                            <td><?= $this->session->userdata('mailing_address')?></td>
                            <input type="hidden" name="mailing_address" id="mailing_address" value="<?= $this->session->userdata('mailing_address')?>" >
                        </tr>
                        <tr>
                            <th>Contact No. :</th>
                            <td><?= $this->session->userdata('contact_no')?></td>
                            <input type="hidden" name="contact_no" id="contact_no" value="<?= $this->session->userdata('contact_no')?>" >
                        </tr>
                        <tr>
                            <th>e-mail ID :</th>
                        ...
                        <tr>
                            ...
                            <td><?= $this->session->userdata('organization_type')?></td>
                            <input type="hidden" name="organization_type" id="organization_type" value="<?= $this->session->userdata('organization_type')?>" >
                        </tr>
                        <tr>
                            <th>Request Submitted by :</th>
                            <td><?= $this->session->userdata('contact_person')?></td>
                            <input type="hidden" name="contact_person" id="contact_person" value="<?= $this->session->userdata('contact_person')?>" >
                        </tr>
                        <tr>
                            <th>Designation :</th>
                            <td><?= $this->session->userdata('designation')?></td>
                            <input type="hidden" name="designation" id="designation" value="<?= $this->session->userdata('designation')?>" >
                        </tr>
                        <tr>
                            <th>Contact No. :</th>
                            <td><?= $this->session->userdata('contact_no')?></td>
...
<script type="text/javascript">
    ...
    $(document).on('change','.checkboxClass', function(){
        ...
        if(this.checked) {
            ...
            rate_id.push($(this).parent().parent().parent().find('.price_<?= $this->session->userdata('organization_type')?>').data('rate_id'));
            prices.push($(this).parent().parent().parent().find('.price_<?= $this->session->userdata('organization_type')?>').val());
            total_price = total_price + (+$(this).parent().parent().parent().find('.price_<?= $this->session->userdata('organization_type')?>').val());
        }else{
            //date.pop(this.value);
            date.splice(date.indexOf(this.value), 1);
            rate_id.splice(rate_id.indexOf($(this).parent().parent().parent().find('.price_<?= $this->session->userdata('organization_type')?>').data('rate_id')), 1);
            prices.splice(prices.indexOf($(this).parent().parent().parent().find('.price_<?= $this->session->userdata('organization_type')?>').val()), 1);
            total_price = total_price - (+$(this).parent().parent().parent().find('.price_<?= $this->session->userdata('organization_type')?>').val());
        }
