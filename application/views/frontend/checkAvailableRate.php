<section class="page">
    <div class="page-header" style="background-image:url(assets/images/banner1.png); position: relative;">
        ...
        <div class="container">
            <h1 class="title"><?= $facilities['location_name']?></h1>
            <h2 class="title"><?= $facilities['sports_facilities_name']?></h2>
        </div>
    </div>
</section>
                ...
                <div class="row">
                    ...
                    <div class="col-xs-3 col-sm-3 wow bounceInUp">
                        <div class="date" data-text="From">
                            <input name="start_date" id="checkin_date" class="datepicker checkIn" value="<?= $this->input->get('start_date')?>" readonly="readonly">
                    ...
                    <div class="col-xs-3 col-sm-3 wow bounceInLeft">
                        <div class="date" data-text="To">
                            <input name="end_date" id="checkout_date" class="datepicker checkOut" value="<?= $this->input->get('end_date')?>" readonly="readonly">
                    ...
                    <?php if(isset($rates) && $rates): foreach($rates as $date => $value):?>
                        <tr>
                            ...
                            <?php foreach($value['rates'] as $rate):?>
                                <td class="text-center">Rs. <?= number_format($rate['rate'],2,",",".")?></td>
                            <?php endforeach?>
                        </tr>
            ...
            <div class="col-sm-12 text-center m_top20">
                <?php if($this->session->userdata('logged_in') && $this->session->userdata('user_type') == 'frontend'):?>
                    ...
                    if($start_date && $end_date):?>
                        <a class="btn btn-main btn_select" href="<?= base_url()?>reserve-facility/<?= $sports_facilities_id?>?start_date=<?= $start_date?>&&end_date=<?= $end_date?>">Proceed to Reserve</a>
                    <?php else:?>
                        <a class="btn btn-main btn_select" href="<?= base_url()?>reserve-facility/<?= $sports_facilities_id?>">Proceed to Reserve</a>
                    <?php endif?>
                <?php else:?>
...
<script type="text/javascript">
    ...
    <?php if($this->input->get('start_date')):?>
        $("#checkin_date").datepicker("setDate", "<?= $this->input->get('start_date')?>");
    <?php else:?>
    ...
    <?php if($this->input->get('end_date')):?>
        $("#checkout_date").datepicker("setDate", "<?= $this->input->get('end_date')?>");
    <?php else:?>
