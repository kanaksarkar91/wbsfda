    ...
    <div style="width: 80%; margin:0 auto; padding-top: 10px;">
    	<div style="background: #fff;box-shadow: 0 2px 5px rgb(0 0 0 / 18%);padding:20px; border-radius:8px;">
                        ...
                        <tr>
                            <td style="width: 10px;">
                                <label class="check_cont">
                                    <input type="checkbox" class="checkboxClass <?= ($month['payment_status']==0)? '':'monthly_subscription_fee'?>" name="monthly_subscription_fee" value="<?= $month['month_year']?>" data-id="<?= @$month['gymnasium_schedule_id']?>" data-amount="<?= @$month['subscription_amount']?>" <?= ($month['payment_status']==0)?'checked="checked" disabled="disabled"':''?> >
                                    <span class="checkmark"></span>
                                </label>
                            </td>
                            <td>Subscription for the month of <?= date('M, Y',  strtotime($month['month_year']))?></td>
                            <td style="text-align: right">Rs. <?= @$month['subscription_amount']?></td>
                            <td style="text-align: right"><span class="badge" <?= ($month['payment_status']==0)? 'style="background: #5cb85c"' : 'style="background: #dc3545"'?>><?= ($month['payment_status']==0)?'Complete':'Pending'?></span></td>
                        </tr>
                    <?php $i++; endforeach?>
                    <tr>
                        <th></th>
            ...
            <div class="col-sm-12" style="float: none;">
                ...
                <input type="hidden" name="surl" id="surl" value="<?= base_url('gymnasium-subscription-success')?>">
                <input type="hidden" name="furl" id="furl" value="<?= base_url('gymnasium-subscription-failure')?>">
