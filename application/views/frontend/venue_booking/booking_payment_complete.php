						...
						<div class="row mb-5">
							...
							<div class="col-lg-12 col-md-12 col-sm-12 text-center">
								<div style="width: 80%; margin:0 auto; padding-top: 30px;" class="text-center">
									...
									<h4 class="text-success">Congratulations! Payment Transaction Successful.<br><br><?= ($booking_id) && $booking_id!= '' ? 'Provisional Booking No. ' . $booking_id : ''; ?><?= isset($txnid) && $txnid != '' ? '<br>Transaction Ref. No. ' . $txnid : ''; ?><?= isset($amount) && $amount != '' ? '<br>Amount ₹' . $amount : ''; ?></h4>
...
<div class="modal hide fade" id="paymentstatusModal">
    ...
    <div class="modal-body text-center">
        <?php if ($status == 'SUCCESS') { ?>
		<!--<h6 class="text-success">Congratulations! Your Payment is successfully completed.<br><br><?= ($booking_id) && $booking_id!= '' ? 'Booking No. ' . $booking_id : ''; ?><?= isset($booking_det->txnid) && $booking_det->txnid != '' ? '<br>Transaction Ref. No. ' . $booking_det->txnid : ''; ?><?= isset($booking_det->amount) && $booking_det->amount != '' ? '<br>Amount ₹' . $booking_det->amount : ''; ?></h6>-->
		<h6 class="text-success">Congratulations!<br> Payment Transaction Successful.<br><br><?= ($booking_id) && $booking_id!= '' ? 'Provisional Booking No. ' . $booking_id : ''; ?><?= isset($txnid) && $txnid != '' ? '<br>Transaction Ref. No. ' . $txnid : ''; ?><?= isset($amount) && $amount != '' ? '<br>Amount ₹' . $amount : ''; ?></h6>
		<?php } elseif ($status == 'FAILURE') { ?>
		...
		<?php 
		if ($status == 'SUCCESS') 
			$lnk = base_url('my-booking?tab=hall-venue');
		if ($status == 'FAILURE')
			$lnk = base_url();
		?>
		<br><br>
        <div class=" text-right"><a href="<?= $lnk; ?>" class="btn btn-primary">OK</a></div>
    </div>
