<section class="section">
    ...
    <div style="width: 80%; margin:0 auto; padding-top: 30px;">
    	<form action="<?php echo base_url('generate-schedule')?>" method="POST" id="schedule-form">
	            ...
	            <table class="table">
	                <tbody>
	                	<tr>
		                    ...
		                    <td align="right">Rs. <?php echo @$gymnasium_rates['registration_fee']?></td>
		                </tr>
		                <?php $i=1; foreach($months_arr as $key => $month):?>
			                <tr>
			                	<td style="width: 10px;">
		                            <label class="check_cont">
		                                <input type="checkbox" class="checkboxClass monthly_subscription_fee" name="monthly_subscription_fee" value="<?php echo $key?>" data-amount="<?php echo @$gymnasium_rates['monthly_subscription_fee']?>" >
		                                <span class="checkmark"></span>
		                            </label>
		                        </td>
			                    <td>Subscription for the month of <?php echo @$month?></td>
			                    <td align="right">Rs. <?php echo @$gymnasium_rates['monthly_subscription_fee']?></td>
			                </tr>
		            	<?php $i++; endforeach?>
		                <tr>
		                    <th>Total Payable</th>
		                    <th></th>
		                    <th style="text-align: right;"><span id="total_payment_txt">Rs. <?php echo @$gymnasium_rates['registration_fee']?></span></th>
		                </tr>
		            </tbody>
		        </table>
	            <div class="checkbox">
    	...
    	<form action="<?php echo base_url('proceed-to-payment')?>" method="POST" id="proceedPayment" style="display: none">
    		...
    		<input type="hidden" name="surl" id="surl" value="<?php echo base_url('gymnasium-success')?>">
            <input type="hidden" name="furl" id="furl" value="<?php echo base_url('gymnasium-failure')?>">
...
<script type="text/javascript">
	$(document).ready(function(){
		let total_payable = 0;
		let registration_fee = parseInt(<?php echo @$gymnasium_rates['registration_fee']?>);
		...
		$(document).on('click', '.proceed_to_pay', function(){
		    ...
		    //return true;
		    $.ajax({
	            url:'<?php echo base_url('generate-schedule')?>',
	            method: 'post',
