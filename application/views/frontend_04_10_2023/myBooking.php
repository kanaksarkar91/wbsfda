                                        ...
                                        <tr>
                                            <td>
                                                <label class="check_cont">
                                                    <input type="checkbox" class="checkboxClass" name="check" value="<?php echo $date['start_date']?>">
                                                    <span class="checkmark"></span>
                                                </label>
                                            </td>
                                            <td class="text-center"><?php echo date('d-m-Y F',strtotime($date['start_date']))?></td>
                        ...
                        <?php if(isset($bookings) && $bookings): ?>
                            <?php foreach($bookings as $booking):?>
                                <tr>
                                    ...
                                    <td class="text-center"><?php echo date('d-m-Y F',strtotime($booking['created_at']))?></td>
                                    <td class="text-center"><?php echo $booking['sports_facilities_name']?></td>
                                    <td class="text-center"><?php echo $booking['location_name']?></td>
                                    <td class="text-center"><?php echo $booking['organization_name']?></td>
                                    <td class="text-center"><?php echo $booking['total_rate']?></td>
                                    <td class="text-center"><?php echo ($booking['booking_status'] == '0')?'Pending':(($booking['booking_status'] == '1') ?'Approved' :(($booking['booking_status'] == '2')?'Rejected' :'Confirmed'))?></td>
                                    <td class="text-center"><?php echo $booking['payment_method']?></td>
                                    <td class="text-center">
                                        <a class="btn-sm app-btn-primary" href="#">View Details</a>
                                        <?php if(($booking['booking_status'] == '1') && ($booking['payment_method'] == 'Online')){ ?>
                                            <button class="btn btn-sm btn-main pay_now" data-booking_id = "<?php echo $booking['booking_id']?>" data-amount="<?php echo $booking['total_rate']?>">Pay Now</button>
...
<form action="<?php echo base_url('proceed-to-payment')?>" method="POST" id="proceedPayment" style="display: none">
    ...
    <input type="hidden" name="surl" id="surl" value="<?php echo base_url('booking-facility-success')?>">
    <input type="hidden" name="furl" id="furl" value="<?php echo base_url('booking-facility-failure')?>">
...
<script type="text/javascript">
    $(document).on('click', '.pay_now', function() {
        ...
        $.ajax({
            url:'<?php echo base_url('generate-txnid')?>',
            method: 'post',
