...
<div class="modal hide fade" id="paymentstatusModal">
    ...
    <div class="modal-body">
        <?php if ($status == 'SUCCESS') { ?>
		<h6 class="text-success">Congratulations! Your Payment is Successful and Booking Confirmed.<br><br><?= isset($booking_det->booking_no) && $booking_det->booking_no != '' ? 'Booking No. ' . $booking_det->booking_no : ''; ?><?= isset($booking_det->txnid) && $booking_det->txnid != '' ? '<br>Transaction Ref. No. ' . $booking_det->txnid : ''; ?><?= isset($booking_det->amount) && $booking_det->amount != '' ? '<br>Amount ₹' . $booking_det->amount : ''; ?></h6>
		<?php } elseif ($status == 'FAILURE') { ?>
