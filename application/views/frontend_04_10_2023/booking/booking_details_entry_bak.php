		...
		<div class="row">
			...
			<div class="col-lg-8 col-md-7">
				...
				<div class="checkout-wrap">
					...
					<form action="<?php echo base_url('frontend/booking/booking_information_entry');?>" method="post" id="bd_entry_form">
					<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
					
					<input type="text" name="day_wise_rates_json" value="<?php print_r($day_wise_rates_json); ?>">
					<input type="text" name="property_id" value="<?= isset($property_id) && $property_id != '' ? $property_id : ''; ?>">
					<input type="text" name="stay_dates" value="<?= isset($stay_dates) && $stay_dates != '' ? $stay_dates : ''; ?>">
					<input type="text" name="adult" value="<?= isset($adult) && $adult != '' ? $adult : ''; ?>">
					<input type="text" name="child" value="<?= isset($child) && $child != '' ? $child : ''; ?>">
					<input type="text" name="rate_category_id" value="<?= isset($rate_category_id) && $rate_category_id != '' ? $rate_category_id : ''; ?>">
					...
					<input type="text" name="day_wise_rates_json" value="<?= isset($day_wise_rates_json) && $day_wise_rates_json != '' ? $day_wise_rates_json : ''; ?>">
					<input type="text" name="property_id" value="<?= isset($property_id) && $property_id != '' ? $property_id : ''; ?>">
					<input type="text" name="stay_dates" value="<?= isset($stay_dates) && $stay_dates != '' ? $stay_dates : ''; ?>">
					<input type="text" name="adult" value="<?= isset($adult) && $adult != '' ? $adult : ''; ?>">
					<input type="text" name="child" value="<?= isset($child) && $child != '' ? $child : ''; ?>">
...
function customerSection() {
	...
	if (guestType == "1") {
		...
		$('#booking_age [value='+ <?php echo $customer_det->age;?> +']').attr('selected', 'true');
		$("#booking_email").val("<?php echo $customer_det->email;?>");
	...
	$("#coupon_action").on("click", function() {
		if ($(this).data('type') == 'apply') {
			...
			if (coupon_code != '') {
				$.ajax({
					...
					method: 'post',
					data: {coupon_code: coupon_code, propertyId: property_id, csrf_test_name: '<?php echo $this->security->get_csrf_hash(); ?>'},
					dataType: 'json',
				}).done(function(data) {
					if (data != null) {
						if (data.success) {
		...
		if ($(this).data('type') == 'remove') {
			$.ajax({
					url: '<?= base_url('frontend/booking/booking_coupon'); ?>',
					method: 'post',
					data: {type: 'remove', csrf_test_name: '<?php echo $this->security->get_csrf_hash(); ?>'},
					dataType: 'json',
				}).done(function(data) {
					if (data != null) {
						if (data.success) {
