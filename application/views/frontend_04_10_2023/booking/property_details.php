							        ...
							        <div class="table-responsive">
										<table class="table table-hover table-bordered mb-0 room-type-tbl">
											...
											<tr>
												...
												<td data-label="Price" class="p-1">₹ <?= isset($accommodation['base_price']) && $accommodation['base_price'] != '' ? $accommodation['base_price'] : ''; ?><?= isset($property['property_type_id']) && !in_array($property['property_type_id'], array(7,8,9,14)) ? '/per room' : ''; ?><br />
												
												<?php /*?><?php
												if($this->uri->segment('7') > $accommodation['adult']){
													if($accommodation['extra_bed_price'] > 1){
												?>
													<input type="radio" value="<?php echo $accommodation['extra_bed_price'];?>" name="choose_extra_pax" id="choose_extra_pax<?= $accommodation['accommodation_id'];?>"  />
													Extra Pax
												<?php
													}
												}
...
$(document).ready(function() {
	$('select[name="no_of_rooms"]').on('change', function() {
		...
		var choose_extra_pax = $('input[name="choose_extra_pax"]:checked').val();
		//console.log(choose_extra_pax);
