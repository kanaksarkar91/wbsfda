...
.category-name.active .title-head,
.category-name.active .desc {
	color: #FFF;
}

.category-name:hover .title-head,
.product-name:hover .title-head,
.category-name:hover .desc,
.product-name:hover .desc {
	color: #FFF;
}
			...
			<div class="col-auto">
				<div class="page-utilities">
					<div class="row g-2 justify-content-start justify-content-md-end align-items-center">
						<!--//col-->
						<div class="col-auto">
							<a class="btn btn-success text-white" target="_blank" href="<?php echo base_url();?>admin/pos/pos_invoice_list/<?php echo encode_url($property_id);?>">Receivable</a>
						</div>
					</div>
					<!--//row-->
				</div>
				...
				<div class="row g-0">
					<div class="col-12 POS-head p-2">
						<div class="row align-items-center">
							<div class="col-md-4">
								<h6 class="mb-0">POS: <?php echo $costCenterData['cost_center_name'];?></h6>
							</div>
							<div class="col-md-4">
								<div class="input-group">
									<span class="input-group-text fw-bold" id="">Table No./ Room No.</span>
									<input type="text" class="form-control" id="table_no">
								</div>
							</div>
							<div class="col-md-4">
					...
					<div class="col-md-3 category-list">
						<div class="POS-head-2 d-flex align-items-center mb-2">
							<input type="text" class="form-control" name="search_cat" id="search_cat" placeholder="Search Category">
						</div>
						<div class="row g-2">
							<div class="col-md-4 col-6" id="cat_list">
					...
					<div class="col-md-6 col-lg-5 item-list">
						<div class="POS-head-2 d-flex align-items-center mb-2">
							<input type="text" class="form-control" name="search_item" id="search_item" placeholder="Search Product Name">
						</div>
						<div class="row g-2">
							<div class="col-md-4 col-6" id="product_list">
					...
					<div class="col-md-3 col-lg-4 ordered-item">
						...
						<div>
							...
							<div>
								  ...
								  <td colspan="4" class="td-pad" >
									<div style="height: 357px; overflow-y: scroll;">
									  <div class="pro-list" id="item_list">
										<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table pro-list-loop">
										  <tr>
							  ...
							  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tr-green">
								<tr>
							...
							<div class="can-hol-pay mt-2">
							  <button type="button" class="btn btn-warning text-white" onClick="holdSale('holdSale')">Hold</button>
...
function getCategories(){
	...
	$.ajax({
		type 		: 'POST',
		url			: '<?php echo base_url("admin/pos/getCategoryList"); ?>',
		data 		: { csrf_test_name: '<?php echo $this->csrf['hash']; ?>', query:query, property_id: <?php echo $property_id;?> },
		dataType 	: 'json',
		encode 		: true
	})
	.done(function(data) {
		...
		if(data.status){
			$.each(data.cat_list, function(key, value){
				catHTML+= '<button type="button" class="btn btn-secondary" onclick="getProducts('+value.category_id+')"><span><strong>' + value.category_name + '</strong></span></button>';
			});
...
function getProducts(category_id = ''){
	...
	$.ajax({
		type 		: 'POST',
		url			: '<?php echo base_url("admin/pos/getProductServiceList"); ?>',
		data 		: { csrf_test_name: '<?php echo $this->csrf['hash']; ?>', query:query, cost_center_id: <?php echo $cost_center_id;?>, category_id: category_id },
		dataType 	: 'json',
		encode 		: true
	})
	.done(function(data) {
		...
		if(data.status){
			$.each(data.product_list, function(key, value){
				prodHTML+= '<button type="button" class="btn btn-primary pro_bor" id="prod_' + value.product_service_id + '" data-product_service_id="' + value.product_service_id + '" data-uom_id="' + value.uom_id + '" data-product_service_name="' + value.product_service_name + '" data-price="' + value.rate + '"><span><strong>' + value.product_service_name + ' / ' + value.uom_name + '</strong></span><br><span class="qt-rs"> Rs ' + value.rate + '</span></button>';
			});
...
function showItems(){
	var itemListHTML = '<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table pro-list-loop">';
	var grandTotal = 0;
	var totqty=0 *1;
	for(var i = 0; i < selectedItems.length; i++){
		var totPrice = 0;
  ...
  var itemName = selectedItems[i].product_service_name;
	...
	itemListHTML+= '<tr id="t_row_' + i + '">' +
					   '  <td width="5%" align="center" valign="middle"><a href="javascript:;" onclick="removeItem(' + i + ');"><i class="fa fa-close"></i></a></td>' +
...
function holdSale(type){
	...
	if($('#table_no').val()!='' && selectedItems.length>0){
	  $.ajax({
		type: 'POST',
		url: '<?php echo base_url("admin/pos/holdPos"); ?>',
		data: {csrf_test_name: '<?php echo $this->csrf['hash']; ?>', selectedItems:selectedItems,table_no:$('#table_no').val(),sale_order_id:$('#sale_order_id').val(),cost_center_id:<?php echo $cost_center_id;?>,booking_id:$('input[name=folio]:checked').val(), property_id: <?php echo $property_id;?>},
		dataType: 'json',
		encode: true,
		async: false
	  })
...
function getOpenFolder(category_id){
	...
	$.ajax({
		type 		: 'POST',
		url 		: '<?php echo base_url("admin/pos/holdPosListing"); ?>',
		data 		: { csrf_test_name: '<?php echo $this->csrf['hash']; ?>', cost_center_id:<?php echo $cost_center_id;?> },
		dataType 	: 'json',
		encode 		: true
	})
	.done(function(data) {
		...
		if(data.status){
			$.each(data.open_folder_list, function(key, value){
				folderHTML += '<tr>';
		folderHTML += '  <td>'+value.order_no+'</td>';
		folderHTML += '  <td>'+value.table_no+'</td>';
		folderHTML += '  <td align="right">'+value.net_bill_amount+'</td>';
		folderHTML += '  <td align="center"><button href="#" class="btn btn-sm btn-primary" onclick="get_details('+value.sale_order_id+',\''+value.order_no+'\',\''+value.table_no+'\')"><i class="fa fa-edit"></i></button></td>';
	  folderHTML += '</tr>';
			});
...
function get_details(sale_order_id,order_no,table_no){
	...
	$.ajax({
		type 		: 'POST',
		url 		: '<?php echo base_url("admin/pos/getFolderDetails"); ?>',
...
function get_open_folio(){
	$.ajax({
		type 		: 'POST',
		url 		: '<?php echo base_url("admin/pos/getOpenFolio"); ?>',
		data 		: { csrf_test_name: '<?php echo $this->csrf['hash']; ?>', property_id: <?php echo $property_id;?> },
		dataType 	: 'json',
		encode 		: true
	})
	.done(function(data) {
...
function transfer(sale_order_id){
  ...
  $.ajax({
	type: 'POST',
	url: '<?php echo base_url("admin/pos/transferToFolio"); ?>',
	data: {action_type: 'transfer_to_folio', csrf_test_name: '<?php echo $this->csrf['hash']; ?>', sale_order_id:sale_order_id, booking_id:$('input[name=folio]:checked').val()},
	dataType: 'json',
	encode: true,
	async: false
  })
  .done(function(data) { 
	if(data.status){
	  window.location.href = '<?php echo base_url();?>admin/pos/pos_invoice/'+sale_order_id;
	}
	 // console.log(data);
  })
  .fail(function(data) {
...
function generate_invoice(sale_order_id){
  ...
  $.ajax({
	type: 'POST',
	url: '<?php echo base_url("admin/pos/transferToFolio"); ?>',
	data: {action_type: 'generate_invoice', csrf_test_name: '<?php echo $this->csrf['hash']; ?>', sale_order_id:sale_order_id},
	dataType: 'json',
	encode: true,
	async: false
  })
