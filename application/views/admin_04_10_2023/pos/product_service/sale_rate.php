        <div class="app-content pt-3 p-md-3 p-lg-3">
            ...
            <?php endif ?>
                ...
                <div class="row g-3 mb-2 align-items-center justify-content-between">
                    <div class="col-auto">
                        <h1 class="app-page-title mb-0">Add Sale Rate: <?php echo $product_service['product_service_name'];?></h1>
                    </div>
                    <div class="col-auto">
                            ...
                            <table class="table app-table-hover mb-0 text-left">
                                ...
                                <?php 
                                    ...
                                    <tr>
                                        ...
                                        ?>
                                            <button class="btn btn-sm app-btn-secondary" onclick="edit_rate(<?php echo $row['product_service_sale_rate_id'];?>)">Edit</button>
                                        </td>
                                        <?php
                                            }
                                        ?>
...
$("#is_taxable").change(function(){ 
	...
	$.ajax({
		type: 'POST',	
		url: '<?php echo base_url("admin/pos/getTaxSacData"); ?>',
		data: {
...
function edit_rate(product_service_sale_rate_id){
	$.ajax({
		type: 'POST',
		url: '<?php echo base_url("admin/pos/getSaleRateData"); ?>',
		data: {csrf_test_name: '<?php echo $this->csrf['hash']; ?>', product_service_sale_rate_id: product_service_sale_rate_id},
		dataType: 'json',
		encode: true
	})
	.done(function(data){
...
function populate_cost_center(selected){
	...
	$.ajax({
		...
		data: {csrf_test_name: '<?php echo $this->csrf['hash']; ?>', property_id: <?php echo $product_service['property_id'];?>, action_type: 'cost_center_data'},
		dataType: 'json',
		encode: true
	})
	//ajax response
...
function populate_tax(selected, is_taxable){
	...
	$.ajax({
		type: 'POST',	
		url: '<?php echo base_url("admin/pos/getAjaxData"); ?>',
		data: {csrf_test_name: '<?php echo $this->csrf['hash']; ?>', is_taxable: is_taxable , action_type: 'tax_data'},
		dataType: 'json',
		encode: true
	})
	//ajax response
...
function populate_sac_code(selected, is_taxable){
	...
	$.ajax({
		type: 'POST',	
		url: '<?php echo base_url("admin/pos/getAjaxData"); ?>',
		data: {csrf_test_name: '<?php echo $this->csrf['hash']; ?>', is_taxable: is_taxable , action_type: 'sac_code_data'},
		dataType: 'json',
		encode: true
	})
	//ajax response
