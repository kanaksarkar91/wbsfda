<div class="app-content pt-3 p-md-3 p-lg-3">
    <div class="container-xl">
        ...
        <div class="row g-3 mb-4 align-items-center justify-content-between">
            <div class="col-auto">
                <h1 class="app-page-title mb-0">Check-In Check-Out List</h1>
            </div>
            <div class="col-auto">
            ...
            <div class="app-card-body">
                <div class="table-responsive">
                        ...
                        <tbody>
                            <?php
                                if(!empty($check_ins)){
                                    foreach($check_ins as $index => $checkIn){
                            ?>
                            <tr>
                                <td class="cell"><?= $index+1 ?></td>
                                <td >
                                    <a href="<?=base_url('admin/booking/booking_details/'.$checkIn->booking_id)?>" type="button" class="btn btn-info" title="Payment Details"><i class="fa fa-eye"></i></a> &nbsp;  
                                    <!-- <a href="" type="button" class="btn btn-primary" title="Edit Booking"><i class="fa fa-pencil"></i></a> &nbsp;  -->
                                    <a target="_blank" href="<?=base_url('admin/booking/downloadInvoice/'.$checkIn->booking_id)?>" type="button" class="btn btn-yellow" title="Download Vowcher"><i class="fa fa-download"></i></a> &nbsp; 
                                    <?php if($checkIn->booking_status != 'C'){ ?>
                                        <?php if($checkIn->checked_in == 0){ ?>
                                            <a href="<?=base_url('admin/reservation/checkin/'.$checkIn->booking_id)?>" type="button" class="btn btn-green">CheckIn</a>
                                        <?php } else { ?> 
                                            <a href="<?=base_url('admin/reservation/checkin_details/'.$checkIn->booking_id)?>" type="button" class="btn btn-green">View CheckIn</a>
                                        <?php } ?>
                                    <?php } ?>
                                </td>
                                <td class="cell"><?= $checkIn->booking_no ?></td>
								<td class="cell"><?= isset($reservation->txnid) && $reservation->txnid != '' ? $reservation->txnid : ''; ?></td>
                                <td class="cell"><?= date('d/m/Y H:i:s A', strtotime($checkIn->created_ts)) ?></td>
                                <td class="cell"><?= $checkIn->property_name ?> </td>
                                <td class="cell"><?= $checkIn->customer_name ?> </td>
                                <td class="cell"><?= $checkIn->mobile ?> </td>
                                <td class="cell"><?= date('d/m/Y', strtotime($checkIn->check_in)) ?> </td>
                                <td class="cell"><?= date('d/m/Y', strtotime($checkIn->check_out)) ?> </td>
                                <td class="cell"><?= $checkIn->booking_for ?></td>
                                ...
                                <td class="cell"><?= $checkIn->net_payable_amount ?></td>                                
                                <td class="cell">
                                    <?php
                                        if($checkIn->booking_status == 'I'){
                                            echo '<span class="btn btn-primary">Initiate</span>';
                                        }else if($checkIn->booking_status == 'A'){
                                            echo '<span class="btn btn-green">Approved</span>';
                                        }else if($checkIn->booking_status == 'C'){
                                            echo '<span class="btn btn-danger">Cancelled</span>';
                                        }else if($checkIn->booking_status == 'O'){
