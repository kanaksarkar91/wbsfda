    ...
    <div class="container-xl" id="container-xl">
            ...
            <div class="col-auto">
                <div class="page-utilities">
                    <div class="row g-2 justify-content-start justify-content-md-end align-items-center">
                        <!--//col-->
                        <div class="col-auto">
                            <a class="btn app-btn-secondary" href="<?= base_url('admin/venue_rates/multi_venue') ?>">
                                VIEW ALL Rate
                            </a>
                        </div>
                    </div>
        ...
        <div class="app-card app-card-settings shadow-sm p-4">
            <form class="settings-form" id="add-rate-form" method="post" action="" enctype="multipart/form-data" autocomplete="off">
                        ...
                        <div class="col-lg-6 col-sm-12 col-md-6 mb-3">
                            ...
                            <select name="property_id" class="form-select select2" id="property_id"required disabled>
                                ...
                                <?php foreach ($properties as $property) { ?>
                                    <option value="<?= $property['property_id'] ?>" <?= set_select('property_id', $property['property_id'], $property['property_id'] == $rate[0]->property_id ? true : false); ?>><?= $property['property_name']; ?></option> 
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-lg-6 col-sm-12 col-md-6 mb-3">
                            ...
                            <select name="selected_venues[]" class="form-select select2" multiple="multiple" id="selected_venues" required>
                                ...
                                <option value="<?= $venue['venue_id']; ?>" <?= in_array($venue['venue_id'], explode(',', $rate[0]->multiple_venue_ids)) ? 'selected' : ''; ?>>
                                    <?= $venue['venue_name']; ?>
                                </option>
                            <?php } ?>
                            </select>
                ...
                <a class="btn app-btn-danger" id="reset-rate-form" href="<?= base_url('admin/venue_rates/multi_edit/'.$rate[0]->rate_id) ?>">CANCEL</a>
            </form>
        </div>
        <!--//app-card-body-->
    </div>
...
<script type='text/javascript'>
    ...
    const venue_ids = "<?= $rate[0]->multiple_venue_ids?>";
    ...
    $(document).ready(function(){
        $('#add-rate-form').submit(function(e){
            ...
            $.ajax({
                url:'<?= base_url("admin/venue_rates/updateRateMultiVenueData/".$rate[0]->rate_id) ?>',
                method: 'POST',
                //data: $(this).serialize(),
                data: {
