		...
		if ($this->input->post()) {
			...
			if(!empty($safari_booking_detail_ids) && is_array($safari_booking_detail_ids)){
				...
				if (!empty($cancellationDetails)) {
					...
					$basePrice = $cancellationDetails['basePrice'];
				}
				
				$response = array('success' => TRUE, "cancel_percent" => $cancel_percent, "cancel_charge" => formatIndianCurrency($cancel_charge), "refund_amt" => $refund_amt, "basePrice" => $basePrice);
			}
			else{
				$response = array('success' => FALSE, "cancel_percent" => 100, "cancel_charge" => formatIndianCurrency($bookingHeaderData['total_price']), "refund_amt" => 0, "basePrice" => $basePrice);
			}
