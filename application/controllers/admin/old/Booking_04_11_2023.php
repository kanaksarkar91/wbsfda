...
class Booking extends MY_Controller
{
	...
	public function book_room_submit()
	{
			...
			foreach ($book_room_qty as $room_key => $room_qty) {
				for ($i = 1; $i <= $room_qty; $i++) {
					...
					foreach($day_wise_rates_decode_data as $row){
						...
						$extra_bed_price = ($this->input->post('is_select_extra_bed')[$room_key] == 1) ? $row['extra_bed_price_b4_disc'] : '0.00';
						$room_cgst = ($this->input->post('is_select_extra_bed')[$room_key] == 1) ? ($row['tax_amount_base_plus_extra'] / 2) : $row['cgst_amount'];
						$room_sgst = ($this->input->post('is_select_extra_bed')[$room_key] == 1) ? ($row['tax_amount_base_plus_extra'] / 2) : $row['sgst_amount'];
						$room_igst = ($this->input->post('is_select_extra_bed')[$room_key] == 1) ? $row['tax_amount_base_plus_extra'] : $row['igst_amount'];
						$room_taxable_amount = ($this->input->post('is_select_extra_bed')[$room_key] == 1) ? ($row['base_price'] + $row['extra_bed_price_b4_disc']) : $row['base_price'];
						
						$_adult = ($this->input->post('is_select_extra_bed')[$room_key] == 1) ? ($this->input->post('adult')[$room_key] + 1) : $this->input->post('adult')[$room_key];
						...
						$booking_detail_data[] = array(
							...
							'is_select_extra_bed' => $this->input->post('is_select_extra_bed')[$room_key],
		...
		if ($this->db->trans_status() === FALSE) {
			...
			$data['booking_payment_listing'] = $this->mcommon->getRow('booking_payment_listing_view', array('booking_id' => $_booking_id));
	 ...
	 * response json
	 */
	public function cancel_booking_bak()
	{
		//print_r($this->input->post());die;
