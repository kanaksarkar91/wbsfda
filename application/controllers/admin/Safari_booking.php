...
class Safari_booking extends MY_Controller
{
	...
	{
		parent::__construct();
		$this->load->model(array('admin/msafari_service', 'mcommon'));
	...
	public function index()
	{
		...
		if($this->admin_session_data['role_id'] == ROLE_SUPERADMIN || check_user_permission($data['menu_id'], 'delete_flag')){
			if($this->admin_session_data['role_id'] != ROLE_SUPERADMIN){
				$services =  $this->mcommon->getDetails('user_safari_service_mapping', array('user_id' => $this->admin_session_data['user_id']));
				$safari_service_header_ids = array_column($services, 'safari_service_header_id');
			}
