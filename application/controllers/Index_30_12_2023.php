...
class Index extends CI_Controller
{
				...
				if(strtotime($customers_otp['expires_at']) <= strtotime(date('Y-m-d H:i:s'))){
					...
					if(!empty($getUser) && $new_password != '' && $confirm_password != ''){
						if($new_password == $confirm_password){
							$result = $this->mcommon->update('master_admin', array('user_id' => $getUser['user_id']), array('password' => md5($new_password)));
					...
					if ($customers_otp) {
						if (strtotime($customers_otp['expires_at']) <= strtotime(date('Y-m-d H:i:s'))) {
							...
							if ($customers_otp['otp'] == $this->input->post('otp')) {
								...
								echo json_encode(array('error' => 1, 'msg' => 'OTP missmatch'));
							}
						}
					} else {
	...
	public function paymentSuccess()
	{
		...
		if ($posted_data) {
			...
			$data['gymnasium_schedule'] = $this->mcommon->getDetails('gymnasium_schedule_temp', array('txnid' => $txnid));
			$user_id = $data['gymnasium_schedule'][0]['user_id'];
			if ($data['gymnasium_schedule']) {
				$payment_id = $this->mcommon->insert('payment', array(
					'type' => 'GYM',
					'user_id' => $user_id,
					'txnid' => $txnid,
					'mihpayid' => $posted_data['mihpayid'],
					'mode' => $posted_data['mode'],
					'status' => $posted_data['status'],
					'unmappedstatus' => $posted_data['unmappedstatus'],
					'cardCategory' => $posted_data['cardCategory'],
					'discount' => $posted_data['discount'],
					'net_amount_debit' => $posted_data['net_amount_debit'],
					'addedon' => $posted_data['addedon'],
					'total_payment_response' => json_encode($posted_data),
					'total_amount' => $posted_data['amount']
				));

				foreach ($data['gymnasium_schedule'] as $gymnasium_schedule) {
					$schedule = array(
						'user_id' => $gymnasium_schedule['user_id'],
						'gymnasium_rate_id' => $gymnasium_schedule['gymnasium_rate_id'],
						'gymnasium_member_id' => $gymnasium_schedule['gymnasium_member_id'],
						'payment_id' => ($gymnasium_schedule['payment_status'] == 0) ? $payment_id : NULL,
						'month_year' => $gymnasium_schedule['month_year'],
						'subscription_amount' => $gymnasium_schedule['subscription_amount'],
						'payment_status' => $gymnasium_schedule['payment_status'],
					);
					$this->mcommon->insert('gymnasium_schedule', $schedule);
				}
				$this->mcommon->delete('gymnasium_schedule_temp', array('txnid' => $txnid));
				$this->mcommon->update('users_gymnasium', array('user_id' => $user_id), array('payment_status' => '1'));

				$user_data = $this->mcommon->getRow('users', array('user_id' => $user_id));
				$session_data = $user_data;
				$session_data['user_type'] = 'frontend';
				$session_data['logged_in'] = TRUE;
				$gymnasium_data = $this->mcommon->getRow('users_gymnasium', array('user_id' => $user_data['user_id']));
				$session_data['gymnasium'] = $gymnasium_data;
				$this->session->set_userdata($session_data);
			}
			$data['payment'] = $posted_data;
			$data['content'] = 'frontend/payment/gym_success';
			$this->load->view('frontend/layouts/index', $data);
		}
	}
	...
	public function paymentSubscriptionSuccess()
	{
		...
		if ($posted_data) {
			...
			//echo $txnid; echo "<br>";
			$data['gymnasium_schedule'] = $this->mcommon->getDetails('gymnasium_schedule_temp', array('txnid' => $txnid));
			//echo "<pre>"; print_r($data['gymnasium_schedule']); echo "<br>";
			$user_id = $data['gymnasium_schedule'][0]['user_id'];
			if ($data['gymnasium_schedule']) {
				$payment_id = $this->mcommon->insert('payment', array(
					'type' => 'GYM',
				...
				foreach ($data['gymnasium_schedule'] as $gymnasium_schedule) {
					...
					$this->mcommon->update('gymnasium_schedule', array('gymnasium_schedule_id' => $gymnasium_schedule['gs_id']), $schedule);
	...
	public function paymentBookingSuccess()
	{
		...
		if ($posted_data) {
			$txnid = $posted_data['txnid'];
			$data['sports_facilities_booking'] = $this->mcommon->getDetails('sports_facilities_booking', array('txnid' => $txnid));
			$user_id = $data['sports_facilities_booking'][0]['user_id'];
			if ($data['sports_facilities_booking']) {
				$payment_id = $this->mcommon->insert('payment', array(
					'type' => 'GYM',
					'user_id' => $user_id,
					'txnid' => $txnid,
				...
				$this->mcommon->update('sports_facilities_booking', array('txnid' => $txnid), array('payment_id' => $payment_id, 'status' => '3'));
			...
			$data['payment'] = $posted_data;
			$data['content'] = 'frontend/payment/facility_success';
			$this->load->view('frontend/layouts/index', $data);
		}
	}
	public function paymentFailure()
	{
		...
		if ($posted_data) {
			...
			$data['content'] = 'frontend/payment/gym_failure';
			$this->load->view('frontend/layouts/index', $data);
		}
	}
	...
	public function paymentBookingFailure()
	{
		...
		if ($posted_data) {
			...
			if ($data['sports_facilities_booking']) {
				...
				$this->mcommon->update('sports_facilities_booking', array('txnid' => $txnid), array('payment_id' => $payment_id, 'status' => '2'));
			...
			$data['payment'] = $posted_data;
			$data['content'] = 'frontend/payment/facility_failure';
			$this->load->view('frontend/layouts/index', $data);
		}
	}
	...
	public function bookingPaymentVerify(){
		...
		$payments = $this->mbooking->get_booking_payment(array("status IN ('PENDING','NOT-FOUND','FAILURE', 'Failure', 'FAILED','AWAITED','INITIATED','UNSUCCESSFUL','Aborted')" => NULL, "booking_header.booking_status IN ('I','F')" => NULL));
					...
					if(!is_null($payment)){
						...
						if ($payment->order_id) {
							...
							if (!empty($obj)) {
								...
								if (((strtotime(date('Y-m-d H:i:s')) - strtotime($payment->created_ts)) > 1020) && ($orderStatus != 'Shipped')) {
...
public function saveVendorPaymentResponse()
    {
		
		$filePath=APPPATH . "/../public/0604RBI1003.R701";
		$myFile = pathinfo($filePath);
		$fileName=$myFile['basename'];
		$isFileNameExists = $this->mreport->checkFileNameExists(array("bank_response_upload.file_name" => $fileName));
		if($isFileNameExists)
		{
			echo 'File already stored in database!';
			$this->mreport->write_activity_log_by_sp('vendor-payment-response-cron' ,'File already stored in database');	 
		}
		else
		{
			$fileUploadDtls=array();
		$fileUploadDtls=array(
			'file_name' => $fileName
		);
		$finalData=array();
		...
		$tempdata2=array();
			try{
				if (($handle = fopen($filePath, "r")) !== FALSE) {
					$bank_response_upload_id=$this->mcommon->insert('bank_response_upload', $fileUploadDtls);
					$tempData1=array(
						'bank_response_upload_id'=>$bank_response_upload_id
					);
					$row = 0;
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE)  {	
					$tempdata2 = $this->mcommon->recursive_change_key($data, array( 0 => 'transaction_type', 1 => 'beneficiary_code',2 => 'beneficiary_name', 3 => 'instrument_amount',4=> 'cheque_number', 5 => 'value_date',6 => 'customer_reference_no', 7 => 'payment_detail_1',8 => 'payment_detail_2', 9 => 'beneficiary_acc_no', 10 => 'bank_ref_no',11 => 'transaction_status', 12 => 'reject_reason',13 => 'ifsc_code', 14 => 'micr_code',15 => 'utr'));		
					$finalData = array_merge($tempData1,$tempdata2);
					
					//echo '<pre>'; print_r($finalData);
					//echo $finalData['instrument_amount'].$row.'<br>';
					
					//echo count($tempdata2); die;
					
					//check special character in array-------------------------
					$special_found = false;
					//$isValid = false;
					$isValid = true;

					$failed_reason = 'Wrong instrument_amount / value_date Found';
					foreach ($finalData as $string) {
						if (preg_match('/[\'^ $%&*()}{@#~?><>,|=_+ ]/', $string)) {
							$special_found = true;
						}
					}
					/*if(count($tempdata2) == BANK_RESPONSE_DATA_VALUE_COUNT){
						if ($special_found) {
							$isValid = false;
							$failed_reason = 'Special Character Found';
						} else {
							if(is_numeric($finalData['instrument_amount']) && isDate($finalData['value_date'])){
								$isValid = true;
							} 
						}
					}
					...
					if($isValid){
						...
						$insert_data = array(
							...
							'beneficiary_code' => $finalData['beneficiary_code'],
							'beneficiary_name' => $finalData['beneficiary_name'],
							'instrument_amount' => $finalData['instrument_amount'],
							'cheque_number' => $finalData['cheque_number'],
							'value_date' => date('Y-m-d', strtotime($finalData['value_date'])),
							'customer_reference_no' => $finalData['customer_reference_no'],
							'payment_detail_1' => $finalData['payment_detail_1'],
							'payment_detail_2' => $finalData['payment_detail_2'],
							'beneficiary_acc_no' => $finalData['beneficiary_acc_no'],
							'bank_ref_no' => $finalData['bank_ref_no'],
							'transaction_status' => $finalData['transaction_status'],
							'reject_reason' => $finalData['reject_reason'],
							'ifsc_code' => $finalData['ifsc_code'],
							'micr_code' => $finalData['micr_code'],
							'utr' => $finalData['utr']
						);
						
						$this->mcommon->insert('bank_response_upload_det', $insert_data);
					} else {
						
						$insert_data = array(
							'file_id' => $bank_response_upload_id,
							'file_line_no' => $row + 1,
							'record_text' => json_encode($finalData),
							'failed_reason' => $failed_reason,
							'created_ts' => date('Y-m-d H:i:s')
						);
						
						$this->mcommon->insert('bank_response_failed', $insert_data);
						
					}
					
					$row++;
				}
					$this->mreport->process_response_data_by_sp($fileName);
				
					$p_process_name='vendor-payment-response-cron';
					$p_log_desc='No of records:'. $row .' stored in database successfully';
					$this->mreport->write_activity_log_by_sp($p_process_name ,$p_log_desc);
					
					//file upload in a folder-------------------
					$getTransactionStatus = array();
					$getTransactionStatus = $this->mreport->get_bank_response_failed_data($bank_response_upload_id);
					$sourcePath = APPPATH . "/../public/".$fileName;
					
					//echo '<pre>'; print_r($getTransactionStatus); die;
					
					if(empty($getTransactionStatus)){
						
						$destinationFilePath = APPPATH . "/../public/success/".$fileName;
						move_file_from_one_to_another_folder($sourcePath, $destinationFilePath);
						$this->mcommon->update('bank_response_upload', array('id' => $bank_response_upload_id), array('upload_status' => 'S'));
						
					} else{
						
						$destinationFilePath = APPPATH . "/../public/failed/".$fileName;
						move_file_from_one_to_another_folder($sourcePath, $destinationFilePath);
						$this->mcommon->update('bank_response_upload', array('id' => $bank_response_upload_id), array('upload_status' => 'F'));
					...
					throw new Exception("Wrong file,file can not opened");
					$this->mreport->write_activity_log_by_sp('vendor-payment-response-cron' ,'Wrong file,file can not opened');
	...
	public function generateVendorPayfile()
	{
		$run_date=RUN_DATE;
		$v_pay_data_array = $this->mreport->getVendorPayDetails(array('b.rundt' => $run_date));
		$bank_forward_feed = $this->mcommon->getRow('bank_forward_feed', array('rundt' => date('Y-m-d')));
		$activity_log =array();
		$commaSeparatedData='';
		...
		{
			$tmpArr = array();
				foreach ($v_pay_data_array as $data) {						
					foreach($data as $key => $val)	
					{
						$data[$key]=(strpos($val, ',') !== false) ?str_replace(',','', $val) :$val ;
					}
					$tmpArr[] = implode(',', $data );
				}
				
				$commaSeparatedData = implode("\n", $tmpArr);
				//echo '<pre>'; print_r($tmpArr); die;
				//echo $commaSeparatedData; die;
				
   			 $dataFile = $bank_forward_feed['feed_file_name']; 
				$activity_log = array(
					'log_datetime' => date('Y-m-d H:i:s'),
					'process_name' => "vendor-pay-file-generation-cron",
					'log_desc' => date('d-m-Y').', No. of records:'.count($tmpArr)
				);    
		 $this->mcommon->insert('activity_log', $activity_log); 
			 //force_download($dataFile,$commaSeparatedData);

			$file_location = APPPATH . "/../public/";
			$myfile = fopen($file_location.$dataFile, "w") or die("Unable to open file!");
			fwrite($myfile, $commaSeparatedData);
			fclose($myfile);
		...
		else
		{
			...
			$activity_log = array(
				'log_datetime' => date('Y-m-d H:i:s'),
				'process_name' => "vendor-pay-file-generation-cron",
				'log_desc' => date('d-m-Y').', No records found'
			);   
			$this->mcommon->insert('activity_log', $activity_log);
			$commaSeparatedData="\n";
			//force_download($dataFile,$commaSeparatedData);
	...
	public function api_to_send_pos_bridge_notification_on_paytm_device($param1)
	{
		...
		$merchantTransactionId = substr(hash('sha256', mt_rand() . microtime()), 0, 20);
