...
class Mpos extends CI_Model {
	...
	public function get_product_service_list($where = array()){
        $this->db->select('product_service_master.*,property_name,category_flag,uom_name,property_master.is_active AS property_status, category_master.is_active AS category_status, uom_master.is_active AS uom_status, cost_center_master.cost_center_name');
		...
		$this->db->join('cost_center_master', 'category_master.cost_center_id = cost_center_master.cost_center_id', 'LEFT');
		if(!empty($where)){
            $this->db->where($where);
        }
	...
	public function getProductServiceList($cost_center_id = 0, $query = '', $category_id = 0)
	{
		...
		$sql = "SELECT pssr.*, psm.*, uom_master.uom_name FROM product_service_sale_rate pssr
				...
				WHERE pssr.cost_center_id = ".$cost_center_id." AND psm.is_active = 1
				AND CURDATE() BETWEEN eff_start_date AND eff_end_date " .$sql_cond;
				
		$sql .= " ORDER BY psm.product_service_name ASC";
		$query = $this->db->query($sql);
		//echo $this->db->last_query(); die;
		//IF(pssr.eff_end_date != '0000-00-00', pssr.eff_end_date >= CURDATE(), pssr.eff_end_date < CURDATE())
        return $query->result_array();
	}
	...
	{
		$sql = "SELECT SUM(psd.payable_amount) AS net_bill_amount, ccm.cost_center_name, psh.sale_order_id, psh.order_no, psh.cost_center_id, psh.table_no, ifnull(DATE_FORMAT(psh.order_generate_time,'%d/%m/%Y'),'N/A') order_generate_time, psh.invoice_no,
		...
		(select ifnull(booking_payment.payment_mode,'') from booking_payment where sale_order_id=psh.sale_order_id AND booking_payment.status = 'SUCCESS' group BY payment_mode)as payment_mode
	...
	public function __get_receivable_invoice_list($property_id = 0, $cost_center_id = 0)
	{
		$sql = "SELECT SUM(psd.payable_amount) AS net_bill_amount, ccm.cost_center_name, psh.sale_order_id, psh.order_no, psh.cost_center_id, psh.table_no, ifnull(DATE_FORMAT(psh.order_generate_time,'%d/%m/%Y'),'N/A') order_generate_time, psh.invoice_no
				...
				LEFT JOIN cost_center_master ccm ON psh.cost_center_id = ccm.cost_center_id
				LEFT JOIN booking_payment bp ON psh.sale_order_id = bp.sale_order_id
				WHERE ((bp.sale_order_id IS NULL) OR (bp.status != 'Success' OR bp.status is null)) AND psh.property_id = ".$property_id." AND psh.cost_center_id = ".$cost_center_id." AND psh.sale_flag = '1' AND psh.booking_id is Null AND psh.invoice_no is not Null
	...
	public function __get_received_invoice_list($property_id = 0, $cost_center_id = 0)
	{
		$sql = "SELECT SUM(psd.payable_amount) AS net_bill_amount, ccm.cost_center_name, psh.sale_order_id, psh.order_no, psh.cost_center_id, psh.table_no, ifnull(DATE_FORMAT(psh.order_generate_time,'%d/%m/%Y'),'N/A') order_generate_time, psh.invoice_no
				...
				LEFT JOIN cost_center_master ccm ON psh.cost_center_id = ccm.cost_center_id
				LEFT JOIN booking_payment bp ON psh.sale_order_id = bp.sale_order_id
				WHERE ((bp.sale_order_id IS NOT NULL AND bp.status = 'Success')) AND psh.property_id = ".$property_id." AND psh.cost_center_id = ".$cost_center_id." AND psh.sale_flag = '1' AND psh.booking_id is Null AND psh.invoice_no is not Null
	...
	public function get_receivable_invoice_payment($sale_order_id = 0)
	{
		$sql = "SELECT ifnull(amount, 0) AS paid_amount, ifnull(booking_payment.payment_mode,'') AS payment_mode
				FROM booking_payment
				
				WHERE sale_order_id = ".$sale_order_id." AND status = 'SUCCESS' LIMIT 1 ";
	...
	public function get_pos_invoice_list_for_cancel($where = array(),$order_by = '', $property_ids = array(), $group_by = null){ 
        $sql = $this->db->select('SUM(psd.payable_amount) AS net_bill_amount, ccm.cost_center_name, psh.sale_order_id, psh.order_no, psh.cost_center_id, psh.table_no, ifnull(DATE_FORMAT(psh.order_generate_time,"%d/%m/%Y"),"N/A") order_generate_time, psh.invoice_no')
                                ->from('pos_sale_detail psd')
								...
								->where('psh.sale_flag = "1" AND ((bp.sale_order_id IS NULL) OR (bp.status != "Success" OR bp.status is null)) AND psh.booking_id is Null AND psh.invoice_no is not Null');
	...
	public function getPosSaleHeaderWithCostCenterCode($where = array()){
        $this->db->select('pos_sale_header.cost_center_id, cost_center_master.cost_center_code');
        $this->db->from('pos_sale_header');
        $this->db->join('cost_center_master', 'pos_sale_header.cost_center_id = cost_center_master.cost_center_id', 'LEFT');
		if(!empty($where)){
            $this->db->where($where);
        }
	...
	public function getMaxInvoiceNo($cost_center_id = 0, $financialStartYear = '', $likeString = ''){
		$sql = "SELECT * FROM (SELECT MAX(invoice_no) AS highest_invoice_number, CASE WHEN MONTH(MAX(order_generate_time)) >= 4 THEN DATE_FORMAT(order_generate_time, '%y') ELSE DATE_FORMAT(order_generate_time, '%y') - 1 END AS financial_year FROM pos_sale_header WHERE cost_center_id = ".$cost_center_id." AND open_status = '1' AND invoice_no LIKE '".$likeString."/%' ) AS temp WHERE temp.financial_year = '".$financialStartYear."' ";
		$query = $this->db->query($sql);
